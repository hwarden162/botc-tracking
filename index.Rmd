...
```{r bmrsetup, include=FALSE}
bmr_data <- data |> 
  filter(script == "Bad Moon Rising")

bmr_num_games <- bmr_data |> 
  filter(result %in% c("Win", "Lose")) |> 
  nrow()

bmr_num_wins <- bmr_data |> 
  filter(result == "Win") |> 
  nrow()

bmr_perc_wins <- (bmr_num_wins/bmr_num_games)*100
```

Row {data-height=200}
-----------------------------------------------------------------------

### Number of Bad Moon Rising Games Played (Since 11/09/2023) 

```{r}
valueBox(bmr_num_games)
```

### Bad Moon Rising Win Percentage
...
### Bad Moon Rising Alignment Distribution
...
### Bad Moon Rising Alignment Win Rate
...
### Bad Moon Rising Character Distribution

```{r}
bmr_data |> 
  mutate(
    starting_character = factor(starting_character, levels = bmr_characters) |>  fct_rev(),
    starting_alignment = factor(starting_alignment, levels = alignments)
  ) |> 
  group_by(starting_alignment, starting_character) |> 
  summarise(
...
### Bad Moon Rising Character Win Rate
